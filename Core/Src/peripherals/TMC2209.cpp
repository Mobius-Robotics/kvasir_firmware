...
T map(T x, T in_min, T in_max, T out_min, T out_max) {
    ...
    U result = (static_cast<U>(x) - in_min) * (static_cast<U>(out_max) - out_min)
...
void TMC2209::setup(UART_HandleTypeDef *new_huart, SerialAddress serial_address) {
    initialize(new_huart, serial_address);
}
...
void TMC2209::enable() {
    if (hardware_enable_port_ != nullptr) {
        HAL_GPIO_WritePin(hardware_enable_port_, hardware_enable_pin_, GPIO_PIN_SET);
    }
...
void TMC2209::disable() {
    if (hardware_enable_port_ != nullptr) {
        HAL_GPIO_WritePin(hardware_enable_port_, hardware_enable_pin_, GPIO_PIN_RESET);
    }
...
void TMC2209::setMicrostepsPerStep(uint16_t microsteps_per_step) {
    uint16_t microsteps_per_step_shifted = constrain_(microsteps_per_step, MICROSTEPS_PER_STEP_MIN,
...
void TMC2209::setAllCurrentValues(uint8_t run_current_percent, uint8_t hold_current_percent,
...
// private
void TMC2209::initialize(UART_HandleTypeDef *new_huart, SerialAddress serial_address) {
...
uint32_t TMC2209::read(uint8_t register_address) {
    ...
    HAL_HalfDuplex_EnableReceiver(huart_);
    auto reply_status = HAL_UART_Receive(huart_, reply_buf, WRITE_READ_REPLY_DATAGRAM_SIZE,
            REPLY_DELAY_MAX_MICROSECONDS / 1000);
...
uint8_t TMC2209::currentSettingToPercent(uint8_t current_setting) {
    uint8_t percent = map(current_setting, CURRENT_SETTING_MIN, CURRENT_SETTING_MAX, PERCENT_MIN,
            PERCENT_MAX);
    return percent;
}
...
uint8_t TMC2209::percentToHoldDelaySetting(uint8_t percent) {
    uint8_t constrained_percent = constrain_(percent, PERCENT_MIN, PERCENT_MAX);
    uint8_t hold_delay_setting = map(constrained_percent, PERCENT_MIN, PERCENT_MAX, HOLD_DELAY_MIN,
            HOLD_DELAY_MAX);
    return hold_delay_setting;
}

uint8_t TMC2209::holdDelaySettingToPercent(uint8_t hold_delay_setting) {
    uint8_t percent = map(hold_delay_setting, HOLD_DELAY_MIN, HOLD_DELAY_MAX, PERCENT_MIN,
            PERCENT_MAX);
    return percent;
}
